...
class ClientRegistry:
    ...
    def get_file_handle(self, fd) -> BinaryIO:
        return self.handles[fd]


class ClientLiveRegistry:
    def __init__(self, basedir, proc_id):
        self.basedir = basedir
        self.proc_id = proc_id
        self.socks: Dict[int, socket.socket] = {}
        self.fds = []
        self.ids: Dict[int, Dict[str, Any]] = {}
        self.renders: Dict[int, IMURender] = {}
        pass

    def register(self, sock: socket.socket, addr, port, out_queue: mp.Queue=None):
        # Registration of New client:
        # - get client socket
        # - store client socket in `socks`
        # - open file handle for this client
        ...
        if fd not in self.renders.keys():
            self.renders[fd] = IMURender(os.path.join(self.basedir, f'process_{str(self.proc_id)}_{fd}.dat'), out_queue=out_queue)
        else:
            logging.warning(f"The fd: {fd} already has related file handle")
    ...
    def flush(self):
        for key, handle in self.renders.items():
            handle.close()

    def get_file_handle(self, fd) -> BinaryIO:
        return self.renders[fd].file_handle
